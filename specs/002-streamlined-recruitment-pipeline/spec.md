...
- **`FR-007`**: Drive setup must define a configurable Cutoff Percentage (%) to qualify for L1.
...
- **`FR-012`**: L1 interviewer can click **"Claim"**, locking candidate to their `user_id` and setting status to `L1_IN_PROGRESS`.
...
- **`FR-016`**: L2 interviewer can click **"Claim"**, locking candidate to `L2_IN_PROGRESS`.
